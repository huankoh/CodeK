...
pretrained_models = ['new_correct_dummy_wgs_best_model_seed0.pt', 'new_correct_dummy_wgs_best_model_seed1.pt']


# Load your pre-trained model here
def load_model(model_path):
    # import pretrained model
    return torch.load('./pretrained/' + model_path, map_location=torch.device('cpu'))
...
def main(args):
    ...
    for model_name in pretrained_models:
        ...
        dataObj = DataContainer(data_name=args.data, data_file=data_file, train_ratio=args.train_prop,
                                ...
                                feats_to_scale=feats_to_scale, seed=seed)
        ...
        print(f"Number of matching samples for {model_name}: {len(matching_samples)}")
        ...
        for i, sample_idx in enumerate(matching_samples):
            ...
            exp = explainer.explain_instance(instance, classifier, num_features=len(feature_names),
        ...
        print(f"Top 30 features for {model_name}:")
        for feat, imp in sorted_features[:100]:
            print(f"{feature_names[feat]}: {imp}")

        top_features_per_model.append(set([feat for feat, imp in sorted_features[:100]]))

        seed += 1

    # Find the overlapped features
    overlapped_features = set.intersection(*[top_features for top_features in top_features_per_model])

    # Print the overlapped features
    print("Overlapped features:")
    for feat in overlapped_features:
        print(f"{feature_names[feat]}")
